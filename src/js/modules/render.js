/**
 * Maximum Secure Static Site - Dynamic Rendering & Encapsulation Module
 * 
 * Implements Text-to-Canvas drawing, closed Shadow DOM mounting, and CSS
 * hierarchy deception.
 */

// 1. Text-to-Canvas Engine: Converts sensitive string metrics into static canvas pixels
export function createSecureTextCanvas(text, fontSpec = 'bold 16px Outfit, sans-serif', color = '#06b6d4', height = 24) {
  const canvas = document.createElement('canvas');
  const ctx = canvas.getContext('2d');
  
  // Set temporary size to measure text length
  ctx.font = fontSpec;
  const metrics = ctx.measureText(text);
  const width = Math.ceil(metrics.width) + 4; // Add padding
  
  // Set real canvas dimensions with device pixel ratio scaling for crystal clear rendering
  ...
  // Scale context to handle retina displays
  ctx.scale(dpr, dpr);
  
  // Clear canvas
  ctx.clearRect(0, 0, width, height);
  
  // Re-apply font specification after resizing canvas
  ctx.font = fontSpec;
  ctx.fillStyle = color;
  ...
  // Block dragging and selecting canvas
...
// 2. Closed Shadow DOM Dashboard Render
// Accepts decrypted JSON dashboard data and dynamically mounts it inside a closed shadow root.
export function renderDashboard(dashboardData, logoutCallback) {
  const host = document.getElementById('dashboard-root');
  if (!host) return;

  // Clear existing landing elements
  ...
  // Load minified stylesheet into Shadow DOM
  const styleLink = document.createElement('link');
  styleLink.rel = 'stylesheet';
  styleLink.href = 'css/style.min.css';
  shadowRoot.appendChild(styleLink);
  
  // Define layout structures with structural CSS Deception (Flexbox order modification, decoy containers)
  container.innerHTML = `
    <div class="max-w-7xl mx-auto w-full">
      ...
      <!-- Main Obfuscation Grid with flex-ordering tricks -->
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        
        <!-- DEC DECEPTION PANEL (order-2 visual, first in DOM tree to confuse visual parsers) -->
        ...
        <!-- SENSITIVE DATABASE AND METRICS (Visual order-1, rendered via canvas to block DOM parsing) -->
        ...
        <!-- DEC DECEPTION PANEL 2 (Dummy decoy statistics, order-3) -->
  ...
  // 3. Render secure texts to Canvas nodes within the Shadow DOM
  ...
  // 4. Wire logout event inside the Shadow DOM
